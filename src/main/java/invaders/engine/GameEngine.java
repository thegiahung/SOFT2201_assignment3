	...
	public void update(){
		...
		for (int i = 0; i < renderables.size(); i++) {
			...
			for (int j = i + 1; j < renderables.size(); j++) {
				...
				if ((renderableA.getRenderableObjectName().equals("PlayerProjectile") && renderableB.getRenderableObjectName().equals("EnemyProjectile") && renderableA.isColliding(renderableB))
					) {
